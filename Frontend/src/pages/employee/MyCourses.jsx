...
import { useEmployeeCourses } from '../../hooks/useEmployeeQueries';
...
const MyCourses = () => {
    ...
    const { data: courses = [], isLoading: loading, error: queryError, refetch } = useEmployeeCourses();
            ...
            <div className="flex h-screen overflow-hidden" style={{ background: "#faf6ef" }}>
                ...
                <div className="flex-1 overflow-auto">
                    <div className="w-full px-8 py-7 flex items-center justify-between flex-shrink-0" style={{ background: '#FAF6EF', borderBottom: '0.5px solid #63472d' }}>
                        <div>
                            <h1 className="text-3xl font-extrabold tracking-tight" style={{ color: '#3D2C1C', fontFamily: 'Georgia, serif' }}>My Courses</h1>
                            <p style={{ color: 'rgba(65, 50, 24, 0.45)', fontSize: 13, marginTop: 4 }}>Track your progress and access course materials</p>
                        </div>
    ...
    if (error && courses.length === 0) {
        return (
            <div className="flex h-screen overflow-hidden" style={{ background: "#faf6ef" }}>
                <EmployeeSidebar collapsed={navCollapsed} onToggle={() => setNavCollapsed(!navCollapsed)} />
                <div className="flex-1 overflow-auto">
                    <div className="w-full px-8 py-7" style={{ background: '#FAF6EF', borderBottom: '0.5px solid #63472d' }}>
                        <h1 className="text-3xl font-extrabold tracking-tight" style={{ color: '#3D2C1C', fontFamily: 'Georgia, serif' }}>My Courses</h1>
                    </div>
                    <div className="px-8 py-6">
                        <div className="rounded-md bg-red-50 border border-red-200 p-6 text-center">
                            <p className="text-lg font-semibold text-red-800">{error}</p>
                            <button onClick={() => refetch()} className="mt-4 h-10 px-6 rounded-lg bg-red-600 text-white font-semibold">Retry</button>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div className="flex rounded-full p-1 gap-1" style={{ background: "#e8e0d4" }}>
                                    {[
                                        ...
                                        <button
                                            ...
                                            style={statusFilter === key ? { background: "#705536", color: "#faf6ef" } : { color: "#6b5e4e", background: "transparent" }}
                                        >
                                            {label}
                                        </button>
                                    ))}
...
const StatPill = ({ label, count, dotColor }) => (
    <div className="flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium" style={{ background: 'rgba(134, 78, 25, 0.08)', color: '#111111' }}>
